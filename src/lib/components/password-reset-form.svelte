		...
		{:else if stage === 'two-factor'}
			{#if registeredTOTP}
				<form onsubmit={submit}>
					...
					<Field.Group>
						<Field.Field class="items-center">
							<Field.Label for="password-reset-totp-{id}" class="sr-only">
								Authenticator code
							</Field.Label>
							<InputOTP.Root
								maxlength={6}
								id="password-reset-totp-{id}"
								class="justify-center"
								bind:value={code}
						...
						<Button
							...
							size="lg"
							class="w-full"
							disabled={pending || passkeyPending || code.length !== 6}
						>
							{#if pending}
								<LoaderCircleIcon class="animate-spin" />
								Verifying…
			...
			{#if registeredTOTP && registeredPasskey}
				<Field.Separator class="my-4 *:data-[slot=field-separator-content]:bg-card">
					Or
				</Field.Separator>
			{/if}
